...
(function () {
  ...
  const state = { prov: '', can: '', par: '', serv: '', mod: '', cats: [1, 2, 3], layer: 'inund', metric: 'n' };
  ...
  Promise.all([
    ...
    UA = ua.rows.map((r) => ({
      codigo: r[0], nombre: r[1], serv: r[2], mod: r[3], prov: r[4], can: r[5], par: r[6], parNombre: r[7], parOrig: r[19],
    ...
    initMap(); initControls(); syncCats(true); update(true);
  ...
  function initControls() {
    ...
    ['inund', 'mm', 'seq', 'inc', 'multi'].forEach((k) => {
      ...
      b.onclick = () => { state.layer = k; ll.querySelectorAll('.layer-opt').forEach((x) => x.classList.toggle('active', x === b)); syncCats(true); update(); };
      ll.appendChild(b);
    });
  ...
  function maxCat(k) { return k === 'multi' ? 3 : HAZ[k].max; }
  function syncCats(reset) {
    const max = maxCat(state.layer);
  ...
  function clearAll() {
    ...
    $('fServ').value = ''; fillMod(); syncCats(true);
    setProv('', true);
  }
  ...
  const exposed = (u, k) => (k === 'multi' ? state.cats.includes(u.inund) || state.cats.includes(u.mm) : state.cats.includes(u[k]));
  ...
  function aggregate(lv, rows) {
    ...
    rows.forEach((u) => {
      ...
      if (exposed(u, state.layer)) { a.exp++; a.usrExp += u.usuarios; }
    });
    return agg;
  }
  ...
  function renderLegend(br, colors, lv) {
    ...
    const allC = state.cats.length === maxCat(state.layer);
  ...
  function renderStats() {
    ...
    const exp = rows.filter((u) => exposed(u, state.layer));
    const hz = HAZ[state.layer].short.toLowerCase();
    $('kUA').textContent = fmt(rows.length);
    $('kExp').textContent = fmt(exp.length);
    $('kExpL').textContent = 'UA expuestas';
    $('kUsr').textContent = fmt(exp.reduce((s, u) => s + u.usuarios, 0));
    $('kNote').textContent = `Cifras de ${HAZ[state.layer].name.toLowerCase()} (categorías: ${catsTxt()}), ${fpct(pct(exp.length, rows.length))} de las UA filtradas. Las barras inferiores muestran las cuatro amenazas.`;
    const n = rows.length;
  ...
  function exportExcel() {
    ...
    const f = [
      ...
      ['Capa del mapa', HAZ[state.layer].name + ' (categorías: ' + catsTxt() + ')'],
      [],
      ['TOTALES'],
      ['Unidades de Atención', n],
      ['UA expuestas a ' + HAZ[state.layer].name.toLowerCase() + ' (categorías seleccionadas)', rows.filter((u) => exposed(u, state.layer)).length],
      ['Usuarios en UA expuestas a ' + HAZ[state.layer].name.toLowerCase(), rows.filter((u) => exposed(u, state.layer)).reduce((s, u) => s + u.usuarios, 0)],
    ...
    // 4. Detalle de UA
    const r4 = [['Código UA', 'Nombre', 'Servicio', 'Modalidad', 'Provincia', 'Cantón', 'Parroquia (SIIMDH)', 'Parroquia DPA (cabecera cantonal/rural)', 'Unidad desconcentrada', 'Región', 'Área', 'Tipo de administración', 'Usuarios', 'Usuarios en extrema pobreza (RS 2025)', 'Usuarios en pobreza (RS 2025)', 'Susc. inundaciones', 'Susc. movimientos en masa', 'Susc. sequía', 'Susc. incendios forestales', 'Multiamenaza (inund. o MM)', 'Expuesta a capa seleccionada']];
    const D = DATA.dict;
    rows.forEach((u) => r4.push([u.codigo, u.nombre, D.serv[u.serv], D.mod[u.mod], NAMES.prov[u.prov], NAMES.can[u.can], u.parOrig, u.parNombre, D.ud[u.ud], D.reg[u.reg], D.area[u.area], D.adm[u.adm],
      u.usuarios, u.extPob, u.pob, CAT[u.inund], CAT[u.mm], CAT[u.seq], CAT[u.inc], u.multi ? 'Sí' : 'No', exposed(u, state.layer) ? 'Sí' : 'No']));
    const ws4 = XLSX.utils.aoa_to_sheet(r4);
    ws4['!cols'] = [10, 38, 30, 34, 16, 18, 22, 24, 40, 10, 9, 24, 9, 12, 12, 14, 14, 12, 14, 12, 14].map((w) => ({ wch: w }));
